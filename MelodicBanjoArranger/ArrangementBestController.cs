...
{
    static class Arrangemenet_Best_engine
    {
        ...
        public static void create_arrangemnets(List<note_node> in_DT_Notes_, int boundry_value)
        {
            //Clear down the arrangement list
            Arrangemenets.clear_arrangements();
            ...
            foreach (note_node current_node in DecisionTree.get_top_nodes())
            {
                ...
                process_node(current_node, starting_cost_, boundry_value);
        ...
        public static void process_node(note_node temp_node_, int current_cost_, int boundry_value)
        {
            ...
            //Return a list sorted by cost asending
            var ascendingCostOrder = DecisionTree.get_Children(temp_node_).OrderBy(i => i.cost);

            int? lowest_cost = null;

            //Work through the child nodes for the passed parent sorted by cost
            foreach (note_node temp_child_ in ascendingCostOrder)
            {

                //Check the cost is within the specified boundary
                if (lowest_cost == null)
                {
                    lowest_cost = temp_child_.cost;
                    ...
                    else
                    {
                        ...
                        process_node(temp_child_, current_cost_, boundry_value);
                ...
                else
                {
                    ...
                    if (Math.Abs(lowest_cost.Value - temp_child_.cost) < boundry_value)
                    {
